...
async function submit() {
  ...
  try {
    ...
    const message = apiMessage(err, "l_Failed_delete_account");
    if (message.toLowerCase().includes("active orders")) {
      error.value = t("l_Delete_account_active_orders");
      return;
    }
      ...
      <div class="safe-sheet relative w-full max-w-md rounded-t-3xl border border-border bg-white shadow-elevated sm:rounded-3xl">
        <header class="border-b border-border px-5 py-4">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <h2 id="delete-account-title" class="text-lg font-bold text-dark">
                {{ t("l_Delete_account") }}
              </h2>
              <p class="mt-1 text-xs text-muted">
                {{ t("l_Delete_account_hint") }}
              </p>
            </div>
            <button
              type="button"
        ...
        <form class="space-y-3 px-5 py-4" @submit.prevent="submit">
          ...
          <div class="flex gap-2 pt-1">
            ...
            <button
              type="submit"
              class="flex-1 rounded-xl bg-error px-4 py-3 text-sm font-semibold text-white transition hover:bg-error/90 disabled:cursor-not-allowed disabled:opacity-60"
              :disabled="submitting"
            >
              {{ submitting ? t("l_Loading") : t("l_Delete_account_confirm") }}
            </button>
          </div>
        </form>
      </div>
