...
async function submit() {
  ...
  try {
    ...
    if (httpStatus(err) === 409) {
      formError.value = 'Меню на сегодня уже существует.'
    } else {
      formError.value = apiMessage(err, 'Не удалось создать меню.')
    }
  } finally {
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        <footer
          class="shrink-0 border-t border-border bg-white/95 px-5 py-4 backdrop-blur-sm sm:rounded-b-3xl"
        >
          <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button
              ...
              @click="submit"
            >
              {{ submitting ? 'Создание…' : 'Создать меню' }}
            </button>
          </div>
        </footer>
      </div>
